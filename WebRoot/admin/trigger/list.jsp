...
<%
	...
	//
	SEntityList listAllTrigger = (SEntityList)request.getAttribute("listAllTrigger");
	int pageNum = listAllTrigger.getPageNum();
	int pageSize =listAllTrigger.getPageSize();
	int totalPage= listAllTrigger.getTotalPage();
	int totalCount = listAllTrigger.getTotalCount();
...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="sdb?opr=triggerManagerService&ac=listAllTrigger" method="post" name="triggerForm" id="triggerForm">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty listAllTrigger}">
						<c:forEach items="${listAllTrigger}" var="item" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${item.columns.id }"  id="id" alt="${item.columns.trigger_name }"/><span class="lbl"></span></label>
								</td>
								<td>${item.columns.trigger_name }</td>
								<td>${item.columns.primary_table_name }</td>
								<td>${item.columns.create_time }</td>
								<%--<td class="center">
										<a class='btn btn-mini btn-group' title="创建触发器" onclick="javascript:alert('预留功能,待开发');"><i class='icon-cog'></i></a>
										<a class='btn btn-mini btn-info' title="编辑" onclick="javascript:alert('预留功能,待开发');"><i class='icon-edit'></i></a>
										<a class='btn btn-mini btn-danger' title="删除" onclick="javascript:alert('预留功能,待开发');"><i class='icon-trash'></i></a>
								</td>
							--%></tr>
						</c:forEach>
					</c:when>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					<a class="btn btn-small btn-success" onclick="add();">创建触发器</a>
				</td>
				<td style="vertical-align:top;">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>/admin/trigger/edit.jsp';
			 diag.Width = 470;
			 diag.Height = 200;
