...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="zxcus">
			<!-- 检索  -->
			<form action="sdb?opr=customerService&ac=searchCus" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						<th class="center">
						<label><input type="checkbox" id="zcheckbox" readonly="readonly"/><span class="lbl"></span></label>
						</th>
						<th>客户名称</th>
						<th>客户简称</th>
						<th>货主</th>
				...
				<c:choose>
					<c:when test="${not empty sEntityList}">
						<c:forEach items="${sEntityList}" var="cus" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<a class="btn btn-mini btn-success" title="选择" onclick="searchBack(${cus.columns.id },'${cus.columns.customer_name }')">
									<i class="icon-ok"></i>
								</a>
								</td>
								<td>${cus.columns.customer_name }</td>
								<td>${cus.columns.customer_shortname }</td>
								<td>${cus.columns.hz_status }</td>
								<td>${cus.columns.hd_status }</td>
								<td>${cus.columns.lx_status }</td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
