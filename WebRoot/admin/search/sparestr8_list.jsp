...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="zxcus">
			<!-- 检索  -->
			<form action="sdb?opr=customerService&ac=getSPARESTR8List" method="post" name="sparestr8Form" id="sparestr8Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${sEntityList}" var="item" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<a class="btn btn-mini btn-success" title="选择" onclick="searchBack('${item.columns.sparestr8 }')"> <i class="icon-ok"></i>
								</a>
								</td>
								<td>${item.columns.code }</td>
								<td>${item.columns.sparestr8 }</td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
