...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="zxcus">
			<!-- 检索  -->
			<form action="sdb?opr=tableAndViewService&ac=listAllTable&type=<%=type %>" method="post" name="tvForm" id="sparestr8Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<input id="backtype" name="backtype" type="hidden"/>
						...
						<c:forEach items="${sEntityList}" var="item" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<a class="btn btn-mini btn-success" title="选择" onclick="searchBack('${item.columns.table_name }','${item.columns.table_type }','${item.columns.comments }')"> <i class="icon-ok"></i>
								</a>
								</td>
								<td>${item.columns.table_name }</td>
								<td>${item.columns.table_type }</td>
...
<script type="text/javascript">
		...
		function searchBack(name,type,comments){
			$("#backname").val(name);
			$("#backtype").val(type);
